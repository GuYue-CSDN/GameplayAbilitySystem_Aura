...
//从游戏上下文出发，逐级获取并返回 UOverlayWidgetController 实例
	...
	UAbilitySystemComponent* ASC, ULoadScreenSaveGame* SaveGame)
{
	...
	const FGameplayEffectSpecHandle SecondaryAttributesSpecHandle = ASC->MakeOutgoingSpec(CharacterClassInfo->SecondaryAttributes_Inifinite, 1.f, SecondaryAttributesContextHandle);
	ASC->ApplyGameplayEffectSpecToSelf(*SecondaryAttributesSpecHandle.Data.Get());
...
//负责从全局数据资产中获取通用技能列表，并将其授予给目标 ASC
...
//用于从一组候选 Actor 中筛选出距离原点最近的前 N 个目标
